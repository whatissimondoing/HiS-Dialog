...
from utils.external_knowledges import MultiWozDB
...
class CollatorTrain(object):
    ...
    def __call__(self, batch):
        ...
        batch_mode_label_ids = []
        batch_size = len(batch)

        for i in range(batch_size):
            if self.cfg.version == "fused" and self.cfg.add_cl_loss:
                encoder_input_ids, span_label_ids, belief_label_ids, resp_label_ids, mode_label_ids = batch[i]
                batch_mode_label_ids.append(mode_label_ids)
            else:
        ...
        if self.cfg.version == "fused" and self.cfg.add_cl_loss:
            batch_mode_label_ids = torch.tensor(batch_mode_label_ids, dtype=torch.long)
            return batch_encoder_input_ids, batch_span_label_ids, batch_belief_label_ids, batch_resp_label_ids, batch_mode_label_ids

        return batch_encoder_input_ids, batch_span_label_ids, batch_belief_label_ids, batch_resp_label_ids


class MultiWOZDataset(Dataset):
    def __init__(self, cfg, reader, data_type, task, ururu, context_size=-1, num_dialogs=-1, train_ratio=1.0, excluded_domains=None):
        super().__init__()
        self.reader = reader
        self.task = task
        self.ururu = ururu
    ...
    def create_turn_batch(self):
        ...
        for dial in tqdm(self.dials, desc='Creating turn batches'):
            ...
            for turn in dial:
                ...
                if self.cfg.version == "fused" and self.cfg.add_cl_loss:
                    self.turn_mode_label_ids.append(self.mode2id[turn["mode"]])
    ...
    def __getitem__(self, index):
        if self.cfg.version == "fused" and self.cfg.add_cl_loss:
            return self.turn_encoder_input_ids[index], self.turn_span_label_ids[index], self.turn_belief_label_ids[index], self.turn_resp_label_ids[
                index], self.turn_mode_label_ids[index]
        else:
            return self.turn_encoder_input_ids[index], self.turn_span_label_ids[index], self.turn_belief_label_ids[index], self.turn_resp_label_ids[
                index]
...
class BaseIterator(object):
    ...
    def get_batches(self, data_type, batch_size, num_gpus, shuffle=False, num_dialogs=-1, excluded_domains=None):
        ...
        turn_bucket = self.bucket_by_turn(dial)  # 按照session中turn_num进行排序
        ...
        for k in turn_bucket:
            # if data_type != "test" and (k == 1 or k >= 17):  # 轮数=1或>=17的去掉，其实只删了十几个session
            #     continue
            if data_type != "test" and (k == 1):  # 轮数=1或>=17的去掉，其实只删了十几个session
                continue
            ...
            for domain, span_info in turn_span_info.items():
                if isinstance(span_info, dict):
                    for slot, spans in span_info.items():
                        ...
                        if t > 0:
                            adjustment += sum([len(c) for c in windowed_context[:t]])
...
class MultiWOZIterator(BaseIterator):
    def __init__(self, reader, device):
        super(MultiWOZIterator, self).__init__(reader, device)
    ...
    def get_data_iterator(self, all_batches, task, ururu, add_auxiliary_task=False, context_size=-1):
        for dial_batch in all_batches:
            ...
            for dial in dial_batch:
                ...
                span_history = []
                for turn in dial:
                    context, span_dict = self.flatten_dial_history(dial_history, span_history, len(turn["user"]), context_size)
                ...
                batch_encoder_input_ids.append(dial_encoder_input_ids)  # (dial_num, turn_num, )
            ...
            # turn first
            batch_encoder_input_ids = self.transpose_batch(batch_encoder_input_ids)  # (turn_num, dial_num)
            ...
            for t in range(num_turns):
                ...
                tensor_span_label_ids = [self.tensorize(b) for b in batch_span_label_ids[t]]
...
class BaseReader(object):
    ...
    def init_tokenizer(self, backbone):
        if "t5" in backbone.lower():
            ...
            tokenizer = BartTokenizer.from_pretrained(backbone, local_files_only=True)
...
class MultiWOZReader(BaseReader):
    ...
    # newly added
    def constraint_dict_to_bsdx(self, constraint_dict):
        tokens = []
        for domain, sv_dict in constraint_dict.items():
            tokens.append("[" + domain + "]")
            for s, v in sv_dict.items():
    ...
    def constraint_dict_to_bsdx_reform(self, constraint_dict):
        tokens = []
        for domain, sv_dict in constraint_dict.items():
            tokens.append("[" + domain + "]")
            for s, v in sv_dict.items():
    ...
    def constraint_dict_to_bspn_reform(self, constraint_dict):
        tokens = []
        for domain, sv_dict in constraint_dict.items():
            tokens.append("[" + domain + "]")
            for s, v in sv_dict.items():
                tokens.append(s + " is " + v + " ,")
        return " ".join(tokens)
...
if __name__ == '__main__':
    reader = MultiWOZReader("t5-base", "2.1")

    iterator = MultiWOZIterator(reader, "cuda")
    train_batches, num_training_steps_per_epoch, _, _ = iterator.get_batches(
        "train", 8, 1, shuffle=True,
        num_dialogs=-1, excluded_domains=None)

    train_iterator = iterator.get_data_iterator(train_batches, "e2e", False, False, -1)
