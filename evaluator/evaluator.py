...
from utils.config import CONFIGURATION_FILE_NAME
from dataset.reader import MultiWOZReader
...
class BLEUScorer:
    ...
    def score(self, parallel_corpus):
        ...
        p0 = 1e-7
        bp = 1 if c > r else math.exp(1 - float(r) / float(c + 1e-8))
...
class MultiWozEvaluator(object):
    ...
    def cal_distinct(self, data, eval_dial_list=None):
        ...
        def calc_distinct_ngram(pair_list, ngram):
            ...
            try:
                result = ngram_distinct_count / ngram_total
            except:
                result = 0
            return result
        ...
        for dial_id, dial in data.items():
            ...
            for turn in dial:
                ...
                if turn["mode"] == "odd":
                    gen = " ".join(turn['resp_gen'].split()[1:-1])
                    # truth = " ".join(turn['redx'].split()[1:-1])
                    gen = self.reader.tokenizer.tokenize(gen)
                    # truth = self.reader.tokenizer.tokenize(truth)
                    pair_list.append(gen)
        if pair_list:
        ...
        return distinct1, distinct2, distinct3

    def distinct(self, data, eval_dial_list=None):
        ...
        # dist1, dist2 = distinct_n_corpus_level(seqs, 1), distinct_n_corpus_level(seqs, 2)
    ...
    def odd_bleu_metric(self, data, eval_dial_list=None):
        gen, truth = [], []
        for dial_id, dial in data.items():
            if eval_dial_list and dial_id not in eval_dial_list:
                continue
    ...
    def e2e_eval(self, data, eval_dial_list=None, add_auxiliary_task=False):
        ...
        if self.reader.version == "fused":
            distinct1, distinct2, distinct3 = self.cal_distinct(data)
            bleu_odd = self.odd_bleu_metric(data)
            # distinct1, distinct2, distinct3 = 0, 0
        else:
            distinct1, distinct2, distinct3 = 0, 0, 0
            bleu_odd = 0
        return bleu, success, match, distinct1, distinct2, distinct3, bleu_odd


class SalesBotEvaluator(object):
    def __init__(self, reader, eval_data_type="test"):
        self.reader = reader
        self.all_domains = definitions.ALL_DOMAINS
    ...
    def odd_bleu_metric(self, data, eval_dial_list=None):
        ...
        for dial_id, dial in data.items():
            ...
            for turn in dial:
                # excepoch <bos_resp>, <eos_resp>
                gen.append(" ".join(turn['resp_gen'].split()[1:-1]))
                truth.append(" ".join(turn['resp'].split()[1:-1]))
    ...
    def e2e_eval(self, data):

        distinct1, distinct2, distinct3 = self.cal_distinct(data)
        bleu_odd = self.odd_bleu_metric(data)

        return distinct1, distinct2, distinct3, bleu_odd
...
if __name__ == "__main__":
    ...
    if cfg.task == "e2e":
        ...
        logger.info('Metrics for ODD: Distinct 1: %.3f, Distinct 2: %.3f,Distinct 3: %.3f, BLEU: %.3f', distinct1, distinct2, distinct3, bleu_odd)
