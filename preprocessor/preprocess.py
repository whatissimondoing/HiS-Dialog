        ...
        for fn, raw_dial in tqdm(list(self.data.items())):
            ...
            for turn_num, dial_turn in enumerate(raw_dial['log']):
                ...
                if not dial_state:  # user
                    ...
                    # --- 若接下来为开放域对话，则将belief state清空
                    # if "general-chat" in dial_turn['dialog_act'] and constraint_dict:
                    #     constraint_dict = OrderedDict()
                    #     constraint_dict['general'] = OrderedDict({'chat': ''})
                    # ---
                    ...
                    for domain, info_slots in constraint_dict.items():
                        if info_slots:
                            ...
                            for slot, value in info_slots.items():
                                if slot == 'chat':  # 我加的
                                    constraints.append('[chat]')
                                    cons_delex.append('[chat]')
                                else:
                    ...
                    single_turn['mode'] = dial_turn['mode']
...
if __name__ == "__main__":
    ...
    parser.add_argument("-version", type=str, default="2.1", choices=["2.0", "2.1", "2.4", "fused"])
